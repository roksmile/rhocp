...
read -p "Enter domain name (e.g., example.com or *.apps.{CLUSTER_NAME}.{BASE_DOMAIN}): " DOMAIN
if [ -z "$DOMAIN" ]; then
...
create_root_ca() {
    if [ ! -f "$ROOT_CA_DIR/rootCA.key" ] || [ ! -f "$ROOT_CA_DIR/rootCA.crt" ]; then
        ...
        openssl req -x509 -new -nodes -key "$ROOT_CA_DIR/rootCA.key" \
            ...
            -subj "/C=KR/ST=State/L=Seoul/O=RedHat/OU=GPS/CN=RootCA" 2>/dev/null || {
